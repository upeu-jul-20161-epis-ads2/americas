...
(function($) {
    ...
    $.KumbiaPHP = {
        ...
        cConfirm: function(event) {
            ...
            footer.append('<button class="btn" data-dismiss="modal" aria-hidden="true">'+translation.ugettext("CANCEL")+'</button>');
            if(este.hasClass('dw-ajax')) {
                if(este.hasClass('subnav2')) {
                    footer.append('<a class="btn btn-success dw-ajax subnav2 dw-spinner" href="'+este.attr("href")+'">'+translation.ugettext("ACEPT")+'</a>');
                }else{
                    footer.append('<a class="btn btn-success dw-ajax dw-spinner" href="'+este.attr("href")+'">'+translation.ugettext("ACEPT")+'</a>');
                }
            } else {
                footer.append('<button class="btn btn-success">'+translation.ugettext("ACEPT")+'</a>');
            }
        ...
        cFRemote: function(event){
            ...
            if(este.hasClass('dw-validate')) { //Para validar el formulario antes de enviarlo
                ...
                if(!$.FormsValidation.validForm(este.attr('name'), confirmation)) {
        ...
        cReport: function(event) {
            ...
            var header = $('<div><button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button><h3><i class="icon-warning-sign" style="padding-right:5px; margin-top:5px;"></i>'+data_title+'</h3></div>').addClass('modal-header');
            var cuerpo = $('<div><p>'+translation.ugettext("In what format you want to see this report?.")+'</p><p>'+translation.ugettext("Remember to recycle paper.")+'</p>'+tmp_form+'</div>').addClass('modal-body');
            var footer = $('<div></div>').addClass('modal-footer');
        ...
        autoload: function(){
            ...
            for(i in $.KumbiaPHP.plugin){
                ...
                $.getScript($.KumbiaPHP.publicPath + 'js/jquery/jquery.' + $.KumbiaPHP.plugin[i] + '.js', function(data, text){});
            }
        },
    ...
    // Inicializa el plugin
    $.KumbiaPHP.initialize('en-us');//es-pe

    var translation=$.KumbiaPHP.translation = {
        trans: 
        {
            'en-us': 
            {
                "In what format you want to see this report?." : "In what format you want to see this report?.",
                "Remember to recycle paper.":"Remember to recycle paper.",
                "CANCEL":"CANCEL",
                "ACEPT":"ACEPT",
            },
        },
        ugettext: function() {
            lang_objx = $.KumbiaPHP.language;
